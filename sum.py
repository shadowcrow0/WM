...
def s2468():
    ...
    for idx, pos in enumerate(pos1s):
        ...
        if dict1[key] == color1s[idx]:
            ...
            rv['diff'].append(key)
            print(rv)
    dataFile = open("%s.csv"%(info['cond']+'_'+info['ID']) + '_'+ str(setsize),'a')
    dataFile.write(str(setsize)+ ',' +info['cond']+',' +info['ID']+','+info['age']+','+info['gender']+ ',' +str(block1s)+','+ rv['diff'] + ','+ rv['same'] + ',' + str(RT) +'\n')
    ...
    for idx, pos in enumerate(pos1s):
        ...
        if dict1[key] == color1s[idx]:
            ...
            print(rv)
    dataFile = open("%s.csv"%(info['cond']+'_'+info['ID']) + '_'+ str(setsize),'a')
    dataFile.write(str(setsize)+ ',' +info['cond']+',' +info['ID']+','+info['age'] +','+info['gender']+ ',' +str(block1s) + ',' + str(block2s) +','+ rv['diff'] + ','+ rv['same'] + ',' + str(RT) +'\n')
########################set4END###############set6START#######################
    win = visual.Window([800,600],color = "black", units = 'pix')
    # 1. define 6 Colors
    ...
    win.flip()
    t5 = core.getTime()
    Ans8_3 = event.waitKeys(keyList = ['k','s'])
    t6 = core.getTime()
    RT3 = t5 - t6
    #reps4
    res4s= []
