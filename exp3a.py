...
gui.DlgFromDict(dictionary=expInfo, title='VWM Task-3A', order=['ID', 'age','gender','block'])
    ...
    def drawContext(self): #draw the context for later selecting
        ...
        if self.context == 0: #circle up,diamond down
            ...
            for j in range(len(self.ups)):
                cir = visual.ImageStim(win = WIN, image='a.jpg', units='pix', size = (30,30))
                circle.append(cir)
            ...
            for j in range(len(self.downs)):
                dim = visual.ImageStim(win = WIN, image='b.jpg', units='pix', size = (30,30))
                diam.append(dim)
...
class Log(Response):
    ...
    def logData(self): #save data
        dataFile = open("{}_{}_{}_{}.csv".format(expInfo['block'],expInfo['ID'] , expInfo['age'] , expInfo['gender']), 'a')
...
def decidePos(sz):
    ...
    if h % 4 == 0:
        up = [ (-200, 100), (-100, 200), (100, -200), (200, 100)]
        ups = sample(up,sz)
        down = [(200, -100), (100, 200), (-100, -200),(-200, -100) ]
        downs = sample(down, sz)
        positionz = list(chain.from_iterable([ups, downs]))
    elif h % 4 == 1:
        up = [ (100, 200), (-100, 200), (-100, -200), (100, -200)] #1234
        ups = sample(up, sz)
        down = [(200, -100),(-200, 100),(-200, -100),(200, 100)] #4231
        downs = sample(down, sz)
        positionz = list(chain.from_iterable([ups, downs]))
    elif h % 4 == 2:
        up = [ (-200, -100),(200, 100),(200, -100),(-200, 100)] #3143
        ups = sample(up, sz)
        down = [ (-100, -200),(100, -200),(100, 200), (-100, 200), ] #3412
        downs = sample(down, sz)
        positionz = list(chain.from_iterable([ups, downs]))
    elif h % 4 == 3:
        up = [  (-200, -100), (-100, 200), (-100, -200),(100, -200)] #3234
        ups = sample(up, sz)
        down = [(100, 200),(200, -100),(-200, 100),(200, 100)] #1421
        downs = sample(down, sz)
        positionz = list(chain.from_iterable([ups, downs]))
...
def studyingList(elem):
    elem.drawContent() #draw color sqaure
...
# pos = decidePos(4)
# cols = decideColor(4)
# elem = Element(pos[0], pos[1], pos[2], cols[0], cols[1], cols[2], cols[3], 1)
# studyingList(elem)
